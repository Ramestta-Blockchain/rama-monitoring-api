...
export const FindOne = async ({ where, info }: UserExtraParams): Promise<User | null> => {
    try {
...
export const FindMany = async ({ where, info, orderBy, skip, offset, limit }: UserExtraParams): Promise<User[]> => {
    try {
        const select = info ? infoExtractForGraphql(info) : []
        const isProduct = select.includes("posts")
        const isCart = select.includes("carts")
